...
- **Exactly-once / duplicate-charge risk** — when a loop calls a side-effecting tool N times, costbomb costs the N−1 repeats as `duplicate_effect_usd` (double-charge exposure if the tool isn't idempotent) and flags it: *"$1,650 in duplicate effects — charge_card×34, only 1 intended"*.
